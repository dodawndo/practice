...
/*
 * FrontController로 요청이 전달되면 비즈니스 로직의 처리 시 Action 클래스로 이동해야하는데
 * 이때, Action 클래스의 메서드를 호출하여 request,response 객체를 전달하고
 * 요청받은 작업을 진행한 후 작업 결과를 공통의 ActionForward 객체 형태로 리턴해야함
 * => 위의 과정들을 각각의 Action 클래스에서 동일한 형태로 수행해야 하므로 코드의 통일성 필요
